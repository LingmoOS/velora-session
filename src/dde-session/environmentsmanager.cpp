...
namespace {
bool isTreelandSession(const QMap<QString, QString> &envMap)
{
    const auto sessionDesktop = envMap.value(QStringLiteral("XDG_SESSION_DESKTOP"));
    if (sessionDesktop.compare(QStringLiteral("Treeland"), Qt::CaseInsensitive) == 0)
        return true;

    const auto currentDesktop = envMap.value(QStringLiteral("XDG_CURRENT_DESKTOP"));
    if (currentDesktop.contains(QStringLiteral("Treeland"), Qt::CaseInsensitive))
        return true;

    const auto compositor = envMap.value(QStringLiteral("DDE_CURRENT_COMPOSITOR"));
    return compositor.compare(QStringLiteral("TreeLand"), Qt::CaseInsensitive) == 0
        || compositor.compare(QStringLiteral("Treeland"), Qt::CaseInsensitive) == 0;
}
}
...
void EnvironmentsManager::createGeneralEnvironments()
{
    ...
    if ((sessionType.isEmpty() || sessionType == "tty") && isTreelandSession(m_envMap)) {
        sessionType = "wayland";
        m_envMap.insert(QStringLiteral("XDG_SESSION_TYPE"), QStringLiteral("wayland"));
    }
    if (m_envMap.value(QStringLiteral("XDG_SESSION_DESKTOP")).isEmpty() && isTreelandSession(m_envMap)) {
        m_envMap.insert(QStringLiteral("XDG_SESSION_DESKTOP"), QStringLiteral("Treeland"));
    }
